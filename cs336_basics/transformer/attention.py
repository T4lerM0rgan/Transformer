...
class MultiheadSelfAttention(torch.nn.Module):
    def __init__(self, d_model: int, num_heads: int, rope: bool = False, theta: float = 10_000, max_seq_len: int = 4_098, device: torch.device = None, dtype: torch.dtype = None, *args: Any, **kwargs: Any):
        ...
        self.q_proj = utils.Linear(self.d_model, self.d_model, device=device, dtype=dtype)
        self.k_proj = utils.Linear(self.d_model, self.d_model, device=device, dtype=dtype)
        self.output_proj = utils.Linear(self.d_model, self.d_model, device=device, dtype=dtype)
        self.v_proj = utils.Linear(self.d_model, self.d_model, device=device, dtype=dtype)
    ...
    def forward(self, x: Float[Tensor, "... seq_count d_model"]):
        q = rearrange(self.q_proj(x), "... seq_count (num_heads d_k) -> ... num_heads seq_count d_k", num_heads = self.num_heads, d_k = self.d_k)
        k = rearrange(self.k_proj(x), "... seq_count (num_heads d_k) -> ... num_heads seq_count d_k", num_heads = self.num_heads, d_k = self.d_k)
        v = rearrange(self.v_proj(x), "... seq_count (num_heads d_k) -> ... num_heads seq_count d_k", num_heads = self.num_heads, d_k = self.d_k)
        seq_count = q.shape[-2]
        if self.isrope:
